# -*- coding: utf-8 -*-
"""
Created on Fri Jun 28 10:06:23 2019

@author: lukas jansing

script to import radiation data
data with 10min resolution from meteoswiss
time span: 01.01.11 00 UTC - 21.05.19 23:50 UTC

afterwards use radiation data to interpolate
...
# Define paths
#---------------------------------------------------------
radpath    = 'add path to MeteoSwiss radiation data'
...
#---------------------------------------------------------
meteoswissmeta = pandas.read_csv(metapath+'\\rad_stations.csv',sep=';')
...
stationnames = ['aadorftänikon','acquarossacomprovasco','adelboden','aigle',
                ...
                'bantiger','baselbinningen','bernzollikofen','beznau','biasca',
                'biere','bischofszell','blattenlötschental',
                'boltigen','buchsaarau','buffalora','bulletlafretaz',
                'cevio','cham','chasseral','chateaudoex','chaumont',
                'chur','cimetta','coldugrandstbernard',
                'crapmasegn','cressier','davos','delemont','disentis',
                'ebnatkappel','eggishorn','egolzwil','einsiedeln',
                'elm','engelberg','evionnaz','evolenevilla','fahy',
                'fribourgposieux','frutigen','genevecointrin',
                'giswil','glarus','gornergrat','gösgen','grächen',
                'grenchen','grimselhospiz','grono','gütschandermatt','güttingen',
                'hallau','hörnli','ilanz','interlaken','jungfraujoch',
                'koppigen','labrevine','lachauxdefonds','ladole',
                'lägern','langnauie','leibstadt','lemoleson','lesattelas',
                'lesdiablerets','locarnomonti','lugano','luzern','magadinocadenazzo',
                'mathod','matro','meiringen','möhlin',
                'montana','montegeneroso','monterosaplattje','mosen','mottec',
                'mühleberg','mühlebergstockeren','nalunsschlivera','napf','neuchatel',
                'nyonchangins','oron','passodelbernina','payerne','pilatus',
                'piotta','pizcorvatsch','pizmartegnas','plaffeien',
                'pully','poschiavorobbia','robiei','rünenberg','salenreutenen',
                'samedan','sanbernadino','säntis','schaffhausen','schüpfheim',
                'scuol','seglmaria','simplondorf','sion','stabio',
                'stamariavalmustair','stchrischona','stgallen','thun','titlis','uetliberg',
                'ulrichen','vaduz','valbella','veveycorseaux','vicosoprano',
...
#---------------------------------------------------------
data_new,stationnames_new = import_meteoswiss_data(stationnames=stationnames,variable='rad',path=radpath,process='no',\
...
#---------------------------------------------------------
# shift rad data into separate container
#---------------------------------------------------------
globalrad = np.zeros((len(stationnames_new),len(datelist_control)))

for i in range(0,len(stationnames_new)):
    globalrad[i,:] = data_new[()][stationnames_new[i]][1,:].astype(float)
    
    # Filtering: Convert unrealistic high values into nans
    for j in range(0,len(datelist_control)):
        if globalrad[i,j] > 2000 or globalrad[i,j] < 0:
            globalrad[i,j] = np.nan
            print('corrected a value!')

# Free the memory...
#del data_new,data_part1,data_part2
del data_new
...
if plotstations == 'yes':
    ...
    # export figure
    saveas = '\stationmap_globalrad.png'
    #plt.show()
...
#---------------------------------------------------------
globalrad_interpolated = np.zeros((len(treenetcoords_lat),len(datelist_control)))
globalrad_unscaled = np.copy(globalrad) # array to rescale stationdata
...
#for i in range(visp_index,visp_index+1):
    ...
    if vertical_interpolation == 'yes':
        ...
        if method == 'empirical_gradient':
            ...
            temp_linreg = np.concatenate((globalrad[nearest[i,0],:],globalrad[nearest[i,1],:],globalrad[nearest[i,2],:]))
            ...
            for j in range(0,3):
                ...
                # Apply gradient
                globalrad[nearest[i,j],:] = globalrad[nearest[i,j],:]+height_diff*slope
    ...
    for j in range(0,len(datelist_control)):
        ...
        #---------------------------------------------------------
        if treenetstationnames[i] in ['Neunkirch_Nord','LWF-Neunkirch1','Neunkirch_SW_Bu','Neunkirch_SW_Ei',
                                      'Sent_9152_Fi','Sent_9152_Foe','LWF-Lens3','Jussy','Neunkirch_SE',
                                      'Neunkirch_N','Neunkirch_SW','Schaenis']:
           ...
           if (np.isnan(globalrad[nearest[i,include_index_1],j]) == False and np.isnan(globalrad[nearest[i,include_index_2],j]) == False):
               globalrad_interpolated[i,j] = globalrad[nearest[i,include_index_1],j]*(distances[i,include_index_2]/(distances[i,include_index_1]+distances[i,include_index_2])) \
               + globalrad[nearest[i,include_index_2],j]*(distances[i,include_index_1]/(distances[i,include_index_1]+distances[i,include_index_2]))
               
           # Options if one of the stations has NaN values
           elif (np.isnan(globalrad[nearest[i,include_index_1],j]) == False):
               globalrad_interpolated[i,j] = globalrad[nearest[i,include_index_1],j]
           elif (np.isnan(globalrad[nearest[i,include_index_2],j]) == False):
               globalrad_interpolated[i,j] = globalrad[nearest[i,include_index_2],j]
               
           # Option if no station actually has values
           else:
               globalrad_interpolated[i,j] = np.nan
        #---------------------------------------------------------   
        ...
        #---------------------------------------------------------
                                        ...
                                        'Birmensdorf','Davos','Laegeren_FF','Laegeren_Hut','Riehen_Forest',
                                        'Riehen_Meteo','Visp']:
            ...
            if (np.isnan(globalrad[nearest[i,closest_index],j]) == False):
                globalrad_interpolated[i,j] = globalrad[nearest[i,closest_index],j]
        ...
        #---------------------------------------------------------
            ...
            if (np.isnan(globalrad[nearest[i,0],j]) == False and np.isnan(globalrad[nearest[i,1],j]) == False and np.isnan(globalrad[nearest[i,2],j]) == False): 
                globalrad_interpolated[i,j] = globalrad[nearest[i,0],j]*weight[i,0] + globalrad[nearest[i,1],j]*weight[i,1] + globalrad[nearest[i,2],j]*weight[i,2]
            ...
            elif (np.isnan(globalrad[nearest[i,0],j]) == True and np.isnan(globalrad[nearest[i,1],j]) == False and np.isnan(globalrad[nearest[i,2],j]) == False):
                globalrad_interpolated[i,j] = globalrad[nearest[i,1],j]*(distances[i,2]/(distances[i,1]+distances[i,2])) + globalrad[nearest[i,2],j]*(distances[i,1]/(distances[i,1]+distances[i,2]))
            
            # Option if station 2 has NaN values but 1 and 3 not
            elif (np.isnan(globalrad[nearest[i,1],j]) == True and np.isnan(globalrad[nearest[i,0],j]) == False and np.isnan(globalrad[nearest[i,2],j]) == False):
                globalrad_interpolated[i,j] = globalrad[nearest[i,0],j]*(distances[i,2]/(distances[i,0]+distances[i,2])) + globalrad[nearest[i,2],j]*(distances[i,0]/(distances[i,0]+distances[i,2]))
        
            # Option if station 3 has NaN values but 1 and 2 not
            elif (np.isnan(globalrad[nearest[i,2],j]) == True and np.isnan(globalrad[nearest[i,1],j]) == False and np.isnan(globalrad[nearest[i,0],j]) == False):
                globalrad_interpolated[i,j] = globalrad[nearest[i,1],j]*(distances[i,0]/(distances[i,1]+distances[i,0])) + globalrad[nearest[i,0],j]*(distances[i,1]/(distances[i,1]+distances[i,0]))
        
            # In case of only one station with values --> just add those (nearest principle)
        
            elif (np.isnan(globalrad[nearest[i,0],j]) == False):
                globalrad_interpolated[i,j] = globalrad[nearest[i,0],j]
            
            elif (np.isnan(globalrad[nearest[i,1],j]) == False):
                globalrad_interpolated[i,j] = globalrad[nearest[i,1],j]
            
            elif (np.isnan(globalrad[nearest[i,2],j]) == False):
                globalrad_interpolated[i,j] = globalrad[nearest[i,2],j]
    ...
    if vertical_interpolation == 'yes':
        for j in range(0,3):
            globalrad[nearest[i,j],:] = globalrad_unscaled[nearest[i,j],:]
...
# Not the case
np.nanmax(globalrad_interpolated,axis=1)
np.nanmin(globalrad_interpolated,axis=1)
    
# Check for amount of values in timeseries
for i in range(0,len(treenetcoords_lat)):
    print(np.where(~np.isnan(globalrad_interpolated[i,:]))[0].shape[0]/len(datelist_control))
...
np.nanmax(globalrad_interpolated[visp_index_wsl,:] - globalrad[visp_index_meteo,:])

#--------------------------------------------------------- 
# Save data for further use
#---------------------------------------------------------
interpolated_globalrad_data = np.concatenate((np.array([datelist_control]),globalrad_interpolated),axis=0)
np.save(radpath+'\interpolated_globalrad_10minres_novertical.npy',interpolated_globalrad_data)