# -*- coding: utf-8 -*-
"""
Created on Fri Jun 28 15:21:21 2019
...
globalrad = import_lwf_data(treenetstation=treenetstation,path=treenetradpath,\
                            variable='globalrad',process_treenet_data='no')
...
for nowdate in datelist_months:
    ...
    wsl_nans                   = np.argwhere(np.isnan(globalrad['treenetglobalrad'][wsl_ind_0:wsl_ind_1+1]))[:,0]
    meteoswiss_nans_novertical = np.argwhere(np.isnan(globalrad['globalrad_interpolated_novertical'][interp_ind_0:interp_ind_1+1]))[:,0]
