# -*- coding: utf-8 -*-
"""
Created on Mon Jul  1 14:21:35 2019

@author: lukas jansing

Script to write csv of existing interpolations in 10min resolution
...
# Define paths
#---------------------------------------------------------
stationpath = 'add path to interpolated data'

#--------------------------------------------------------- 
# Define variables and interpolation types
#---------------------------------------------------------
variables = ['precipitation','temperature1','temperature2','temperature3',
             'relative_humidity','radiation']
#variables = ['temperature1']
types     = ['novertical','standardgradient','empiricalgradient']
...
for variable in variables:
    ...
    #---------------------------------------------------------
    print('Import interpolated data of '+variable)
    if variable == 'precipitation':
        interpolated_data = np.load(stationpath+'\\'+variable+'\interpolated_'+variable+'_10minres_newmethod.npy')
    if variable == 'temperature1':
        interpolated_data = np.load(stationpath+'\\'+variable[:-1]+'\interpolated_'+variable[:-1]+'_10minres_novertical.npy')
    if variable == 'temperature2':
        interpolated_data = np.load(stationpath+'\\'+variable[:-1]+'\interpolated_'+variable[:-1]+'_10minres_standardgradient.npy')
    if variable == 'temperature3':
        interpolated_data = np.load(stationpath+'\\'+variable[:-1]+'\interpolated_'+variable[:-1]+'_10minres_empiricalgradient.npy')
    if variable == 'relative_humidity':
        interpolated_data = np.load(stationpath+'\\'+variable+'\interpolated_relhum_10minres_novertical.npy')
    if variable == 'radiation':
        interpolated_data = np.load(stationpath+'\\'+variable+'\interpolated_globalrad_10minres_novertical.npy')
    ...
    if variable == 'temperature1' or variable == 'temperature2' or variable == 'temperature3':
        pd.DataFrame(interpolated_data_new).to_csv(stationpath+'\\'+variable[:-1]+'\interpolated_'+variable[:-1]+'_10minres_'+types[k]+'.csv',\
                    header=None,index=None,na_rep='NaN',sep=';')
        k += 1
    else:
        pd.DataFrame(interpolated_data_new).to_csv(stationpath+'\\'+variable+'\interpolated_'+variable+'_10minres.csv',\
            header=None,index=None,na_rep='NaN',sep=';')