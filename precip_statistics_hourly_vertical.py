# -*- coding: utf-8 -*-
"""
Created on Mon Jul  8 09:46:41 2019

@author: lukas jansing

Script to compare if interpolation improves when including vertical scaling
...
#---------------------------------------------------------
print('import interpolated precipitation data (horizontal only)')
interpolated_precipitation_data_horizontalonly = np.load(stationprecippath+'\interpolated_precipitation_newmethod.npy')

print('import interpolated precipitation data (including vertical)')
#interpolated_precipitation_data = np.load(stationprecippath+'\interpolated_precipitation_standardgradient.npy')
interpolated_precipitation_data = np.load(stationprecippath+'\interpolated_precipitation_version4.npy')
precip = {}
...
#--------------------------------------------------------- 
# Define differences between the two interpolations
#---------------------------------------------------------
diff = []
for i in range(1,interpolated_precipitation_data[()].shape[0]):
    diff.append(np.nanmax(interpolated_precipitation_data[()][i,:] - \
                interpolated_precipitation_data_horizontalonly[()][i,:]))
diff = np.array(diff)

#--------------------------------------------------------- 
# Load TreeNet/LWF data
#---------------------------------------------------------
precip_treenet = import_lwf_precipitation_data(treenetstation=treenetstation,treenetprecip_res=treenetprecip_res,\
                                               path=treenetprecippath,process_schänis=process_schänis)
precip.update(precip_treenet)
